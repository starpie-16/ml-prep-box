...
def apply_scaling(series, method = 'standard'):
  scaler = RobustScaler() if method == 'robust_scale' else StandardScaler()
...
def apply_imputation(series, dtype):
  ...
  imputed_data = imputer.fit_transform(series.values.reshape(-1, 1))

  return imputed_data.flatten(), imputer
